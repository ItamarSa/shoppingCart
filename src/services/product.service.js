...
function getImgs() {
    const randomIndex = utilService.getRandomIntInclusive(0, productImages.length - 1)
    return productImages[randomIndex]
}


function getDemoProduct() {
    const imgSet = getImgs() // Use the getImgs function to get a random set of images
    const imgUrl = imgSet[0] // Accessing the first image in the set
